...
void D_MAKRO_CiliaSphereTracker::Save_AnalysisAll()
{
    ...
    DIR_SaveStackGraphics_Chaos.setPath(DIR_SaveStackGraphics.path() + "/Chaos");                   QDir().mkdir(DIR_SaveStackGraphics_Chaos.path());
...
void D_MAKRO_CiliaSphereTracker::Save_AnalysisSingle()
{
    ...
    DIR_SaveCurrentTables.setPath(DIR_SaveCurrent.path() + "/Tables");            QDir().mkdir(DIR_SaveCurrentTables.path());
    ...
    */
    ...
    //stream chaos values to table
    QString QS_Chaostable_Path = DIR_SaveCurrentTables.path() + "/" + name_current + " - TrackChaos.csv";
    ofstream OS_Chaos;
    OS_Chaos.open(QS_Chaostable_Path.toStdString());
    if(OS_Chaos.is_open())
    {
        OS_Chaos << "Track Chaos of " << name_current.toStdString();
        OS_Chaos << "\n" << QS_Chaostable_Path.toStdString();
        OS_Chaos << "\n" << "ImageD Version;" << D_QS_Version.toStdString() << ";Released;" << D_QS_Release.toStdString();
        OS_Chaos << "\n" << "Anlysis done;" << QDateTime::currentDateTime().toString().toStdString();
        OS_Chaos << "\n";
        OS_Chaos << "\n" << "weight type;detections thres;res X;res Y;chaos value";
    }

    //param combinations
    const vector<int> vChaos_ResX       = {5, 10, 20};
    const vector<int> vChaos_ResY       = {4, 8, 16};
    const vector<int> vChaos_DetThres   = {0, 10, 100, 1000};

    //overview text about track chaos
    QString QS_TrackChaos = "Track chaos";

    //loop param combos
    for(size_t w = FIELD_SUMMARY_WEIGHT_1; w < FIELD_SUMMARY_WEIGHT_NUMBER_OF; w++)
    {
        QS_TrackChaos.append("\n\nWeight type: " + QSL_FieldSummaryWeights[w] + "\n");

        for(size_t i_thres = 0; i_thres < vChaos_DetThres.size(); i_thres++)
        {
            int thres = vChaos_DetThres[i_thres];
            QS_TrackChaos.append("\nMinimum number of detetcions per gridcell required: " + QString::number(thres));

            for(size_t i_res = 0; i_res < vChaos_ResX.size(); i_res++)
            {
                int res_x = vChaos_ResX[i_res];
                int res_y = vChaos_ResY[i_res];

                //set params
                ui->spinBox_ParamGridHorizontal->setValue(res_x);
                ui->spinBox_ParamGridVertical->setValue(res_y);
                ui->spinBox_Res_FieldSummary_DetectionsThresh->setValue(thres);
                ui->comboBox_Res_FieldSumary_WeightType->setCurrentIndex(w);
                Update_Ui();

                //get chaos value
                double chaos = ui->doubleSpinBox_FieldSumary_StatResult->value();
                if(chaos < 0 || chaos > 1)
                    chaos = NAN;
                ...
                //table
                if(OS_Chaos.is_open())
                    OS_Chaos << "\n" << QSL_FieldSummaryWeights[w].toStdString() << ";" << thres << ";" << res_x << ";" << res_y << ";" << chaos;

                //imgs
                QString QS_ParamsAndChaos =
                        "WeightType_"   + QSL_FieldSummaryWeights[w] +
                        " DetThres_"    + QString::number(thres) +
                        " Res_"         + QString::number(res_x) + "x" + QString::number(res_y) +
                        " Chaos_"       + QString::number(chaos).replace(".", "_").replace(",", "_");

                View_Results.Save_Image(DIR_SaveCurrentGraphics.path() + "/" + name_current + " - " + QS_ParamsAndChaos + ".png");
                View_Results.Save_Image(DIR_SaveStackGraphics_Chaos.path() + "/" + QS_ParamsAndChaos + " - " + name_current + ".png");

                //chaos strings for pdfs
                QS_TrackChaos.append("\nGrid resolution " + QString::number(res_x) + "x" + QString::number(res_y) + ": " + QString::number(chaos * 100.0, 'g', 4) + "%");
            }
        }
    }
    OS_Chaos.close();

    //default versions of chaos to be saved in summary pdf
    ui->spinBox_ParamGridHorizontal->setValue(10);
    ui->spinBox_ParamGridVertical->setValue(8);
    ui->spinBox_Res_FieldSummary_DetectionsThresh->setValue(1000);
    //1
    ui->comboBox_Res_FieldSumary_WeightType->setCurrentIndex(FIELD_SUMMARY_WEIGHT_1);
    Update_Ui();
    double chaos_10x8_w1 = ui->doubleSpinBox_FieldSumary_StatResult->value();
    //count
    ui->comboBox_Res_FieldSumary_WeightType->setCurrentIndex(FIELD_SUMMARY_WEIGHT_COUNT);
    Update_Ui();
    double chaos_10x8_wCount = ui->doubleSpinBox_FieldSumary_StatResult->value();
    //speed
    ui->comboBox_Res_FieldSumary_WeightType->setCurrentIndex(FIELD_SUMMARY_WEIGHT_SPEED);
    Update_Ui();
    double chaos_10x8_wSpeed = ui->doubleSpinBox_FieldSumary_StatResult->value();

    //set nan if it makes no sense
    if(chaos_10x8_w1     < 0 || chaos_10x8_w1     > 1)  chaos_10x8_w1       = NAN;
    if(chaos_10x8_wCount < 0 || chaos_10x8_wCount > 1)  chaos_10x8_wCount   = NAN;
    if(chaos_10x8_wSpeed < 0 || chaos_10x8_wSpeed > 1)  chaos_10x8_wSpeed   = NAN;
    ...
    QS_Stats_Angle.append(
                "\n--------------------------------------\n"
                "Track chaos 10x8 n>1000 w=1 " + QString::number(chaos_10x8_w1 * 100.0, 'g', 4) + "%\n"
                "Track chaos 10x8 n>1000 w=n " + QString::number(chaos_10x8_wCount * 100.0, 'g', 4) + "%\n"
                "Track chaos 10x8 n>1000 w=v " + QString::number(chaos_10x8_wSpeed * 100.0, 'g', 4) + "%");
    ...
    //track chaos
    PDF_Overview.add_NewPage();
    PDF_Overview.add_Text(QS_TrackChaos);

    //short
    PDF_Summary.add_Text(
                "Angle Stats:\n"
                "Average " + QString::number(v_VideoStats_Angles_Grad[c_STAT_CIRC_MEAN_ANG], 'g', 4) + "°\n"
                "Balance/Chaos " + QString::number(v_VideoStats_Angles_Grad[c_STAT_CIRC_BALANCE] * 100.0, 'g', 4) + "%\n"
                "STD Equivalent " + QString::number(v_VideoStats_Angles_Grad[c_STAT_CIRC_BALANCE_PI_OR_180_1SIGMA], 'g', 4) + "°\n"
                "Chaos 10x8 n>1000 1 " + QString::number(chaos_10x8_w1 * 100.0, 'g', 4) + "%\n"
    ...
    //grid cell info for field summary (graphics for sumary pdf)
    ui->comboBox_Res_Type->setCurrentIndex(RES_GRAPHICS_FIELD_SUMMARY);
    ui->spinBox_Res_FieldSummary_DetectionsThresh->setValue(1000);
    ui->comboBox_Res_FieldSumary_WeightType->setCurrentIndex(FIELD_SUMMARY_WEIGHT_1);
    ...
    PDF_Overview.add_Image(
                ...
                "\n"
                "\nLines:"
                "\nBalance (0: all alined, 1: maximum chaos)"
                "\nn = Number of movement measurements"
                "\nv = Mean speed"
                "\nw = Enough measurements to be used for track chaos calculation?");
